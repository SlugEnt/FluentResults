...
namespace SlugEnt.FluentResults.Extensions.AspNetCore
{
    public class FailedResultToActionResultTransformationContext
    {
        ...
        public FailedResultToActionResultTransformationContext(ResultBase result) { Result = result; }
    }
        ...
        where TResult : ResultBase
    {
        ...
        public OkResultToActionResultTransformationContext(TResult result) { Result = result; }
    }
    ...
    public class ResultToActionResultTransformer
    {
        public ActionResult Transform(Result result,
                                      IAspNetCoreResultEndpointProfile profile)
        {
            if (result.IsFailed)
            {
            ...
            return
                profile.TransformOkNoValueResultToActionResult(new OkResultToActionResultTransformationContext<Result>(result));
        }


        public ActionResult Transform<T>(Result<T> result,
                                         IAspNetCoreResultEndpointProfile profile)
        {
            if (result.IsFailed)
            {
            ...
            return
                profile.TransformOkValueResultToActionResult(new OkResultToActionResultTransformationContext<Result<T>>(result));
        }
    }
}